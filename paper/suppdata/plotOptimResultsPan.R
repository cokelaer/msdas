...
# aliases
    ...
    for(c in 1:dim(CNOlist@signals[[1]])[2]) {
        ...
        for (i in seq_along(labels)){
            labels[i] = gsub("FUS3_S177-T180~Y182", "FUS3_T180~Y182", labels[i])
            labels[i]=  gsub("STE11_S326", "STE11_S323", labels[i])
            labels[i] = gsub("-", "+", labels[i])
            labels[i] = gsub("~", "_", labels[i])
        }
        ...
        text(
            ...
            y=(yMin+((yMax-yMin)/2)),
            cex=cex, srt=plotParams$rotation
        )
    ...
    for(c in 1:dim(CNOlist@signals[[1]])[2]) {
        ...
        for(r in 1:dim(CNOlist@signals[[1]])[1]) {
            ...
            ############ do we need xticks and yticks on main colormesh axes ?
            # add the axis annotations: if we're on the last row, add the x axis
            if(r == dim(CNOlist@signals[[1]])[1]) {
                #axis(side=1,at=CNOlist@timepoints, cex=0.5)
            }
        ...
        if (length(CNOlist@stimuli) == 0){
            ...
            if(c1 == dim(CNOlist@signals[[1]])[1]) {
                axis(
                    ...
                    labels=colnames(CNOlist@stimuli),las=3,cex.axis=2
                )
            }
        }
        sStim = sStim+1
...
#       par(fg="blue",mar=c(0.5,0.5,0.7,0))
    ...
    plot(
       ...
       text(
        labels = "Error",
        x = ((xVal[length(xVal)]-xVal[1])/2),
        y = (yMin+((yMax-yMin)/2)),cex=cex*1.2
        )
    ...
    rhs <- 1
    rhs2 <- rhs + rhs/10
    at <- c(-0.020, 0.5, 1.015)
    # margin of colobar bottom left   top right
    par(mai=c(0.,0.0,0.0,0.0))
    #plot.new()

    tryCatch({
        plot.new()
        yyy <- seq(0,1,length=len+1)
        rect(0, yyy[1:len], rep(rhs, len), yyy[-1], col = colbar, border =
             colbar)
        rect(0, 0, rhs, 1, border="black")
        segments(rhs, at, rhs2, at)
        #text(x=.8, y=at, labels=labels, pos=4, offset=0.0, cex=plotParams$cex)
